...
function renderChecklist() {
  ...
  // LMRA-Box: Mitarbeiter + Datum
  const lmraMaBox = document.getElementById('lmra-ma-box');
  if (lmraMaBox) lmraMaBox.style.display = isLMRA ? 'block' : 'none';
  if (isLMRA) {
...
async function submitChecklist() {
  ...
  if (!document.getElementById('pruefer-name').value.trim()) {
    ...
    alert(isGFB ? 'Bitte Name des Aufsichtsführenden eingeben.' : (isLMRA2 ? 'Bitte Name des Aufsichtsführenden eingeben.' : 'Bitte Name des Prüfers eingeben.'));
    return;
  }
...
async function generatePDF() {
  ...
  if (isLMRApdf) {
    // ===== LMRA: Mitarbeiter-Unterschriftenseite =====
    doc.addPage(); y = PT;
    doc.setFillColor(26, 58, 92); doc.rect(0, 0, 210, 22, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(14); doc.setFont('helvetica', 'bold'); doc.text('LMRA – LAST-MINUTE-RISIKOANALYSE', PL, 10);
    doc.setFontSize(9); doc.setFont('helvetica', 'normal');
    doc.text('Hubarbeitsbühne · Glas- und Fassadenreinigung', PL, 17);
    doc.setFontSize(8); doc.text('Erstellt gemäß BG BAU · Jeder Mitarbeiter bestätigt, die LMRA verstanden zu haben.', PL, 21);
    doc.setTextColor(0); y = 28;
    ...
    doc.text('Ich habe die LMRA durchgeführt und alle Punkte verstanden. Die Arbeit ist aus meiner Sicht sicher.', PL, y, { maxWidth: PW });
    y += 10; doc.setTextColor(0);
